...
function Newchat( userid ) {
    ...
    $.ajax({
        ...
        success:function( data ) {
            ...
            if( d["chatid"] && d["chatid"] !== undefined ) {
                elem = document.createElement( "li" );
                elem.className = "list-group-item";
                elem.setAttribute( "onclick", "openChat('"+d["chatid"]+"', '"+userid+"');" );
                elem.textContent = name;

                document.getElementById("newchat").style.display = "none";
                document.getElementById("user_list").children[0].children[0].appendChild( elem );
                openChat( d["chatid"], userid );
            }
            else {
                alert( "Не удалось создать групповой чат! postResp[\"chatid\"] неопределён." );
            }
        }
    });
}
...
function displayChat( dispdata, userid ) {
	...
	document.getElementById( "pmchats" ).style.display = "none";
    //document.getElementById("groupchatlist").style.display = "none";
...
function openPM( id ) {
	document.getElementById("chatarea").style.display = "none";
    let iframe = document.getElementById( "pmchats" );
    //document.getElementById("groupchatlist").style.display = "none";

    iframe.src = "private_chat.php?id="+id;
