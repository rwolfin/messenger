...
if( isset($_POST["action"]) && $_POST["action"] == "newchat" ) {
	require('database/ChatUserModel.php');

	$user_object = new ChatUserModel;
	$chatid = $user_object->newGroup( $_POST["chatname"], $_POST["user_id"] );
	
	echo json_encode( array("chatid" => $chatid) );
}

if( isset($_POST["action"]) && $_POST["action"] == "getchat" ) {
	require('database/ChatUserModel.php');

	$user_object = new ChatUserModel;
	$res = $user_object->getGroupMessages( $_POST["chat_id"] );
	for( $i = 0; $i < count($res); $i++ ) {
		$user_object->setUserId( $res[$i]["userid"] );
		$username = $user_object->getUsernameById( $res[$i]["userid"] );
		$emails = $user_object->get_user_data_by_id();
		//if( $res[$i]["userid"] == $_POST["user_id"] ) { $res[$i]["uname"] = "Me"; continue; }

		if( $emails["showemail"] ) $res[$i]["uname"] = $emails["user_email"];
		else $res[$i]["uname"] = $username;
	}

	$res[0]["title"] = $user_object->getGroupById( $_POST["chat_id"] )[0]["chatname"];
	
	echo json_encode( $res );
}

if( isset($_POST["action"]) && $_POST["action"] == "sendgrpmsg" ) {
	require('database/ChatUserModel.php');

	$user_object = new ChatUserModel;
	$user_object->sendChatMessage( $_POST["userid"], $_POST["group_chat_message"], $_POST["groupid"] );
}

if( isset($_POST["action"]) && $_POST["action"] == "addutogc" ) {
	require('database/ChatUserModel.php');

	$user_object = new ChatUserModel;
	$res = $user_object->addUserToGC( $_POST["user_id"], $_POST["chat_id"] );

	if( $res ) echo json_encode( array("retn", 1) );
	else echo json_encode( array("retn", 0) );
}

if( isset($_POST["action"]) && $_POST["action"] == "updatemsg" ) {
	require('database/ChatUserModel.php');

	$user_object = new ChatUserModel;
	$user_object->updateMessage( $_POST["msgid"], $_POST["msg"] );
}

if( isset($_POST["action"]) && $_POST["action"] == "updatemsgp" ) {
	require('database/ChatUserModel.php');

	$user_object = new ChatUserModel;
	$user_object->updatePMessage( $_POST["msgid"], $_POST["msg"] );
}

if( isset($_POST["action"]) && $_POST["action"] == "removemsgp" ) {
	require('database/ChatUserModel.php');
	
	$user_object = new ChatUserModel;
	$user_object->removePMessage( $_POST["msgid"] );
}

if( isset($_POST["action"]) && $_POST["action"] == "removemsg" ) {
	require('database/ChatUserModel.php');
	
	$user_object = new ChatUserModel;
	$user_object->removeMessage( $_POST["msgid"] );
}
