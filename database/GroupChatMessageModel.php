...
class GroupChatMessageModel
{
	...
	public function openchat( $id ) {
		...
		if( count($res) > 0 ) { $this->group_id = $id; return TRUE; }
		else return FALSE;
	}
	...
	function setGroupId( $id ) {
		file_put_contents( "logs.txt", print_r($this->group_id, TRUE).PHP_EOL.PHP_EOL.PHP_EOL, FILE_APPEND );
		$this->group_id = $id;
	}
	...
	function save_chat()
	{
		$query =
			...
			(userid, group_chat_message, groupid, created_on) VALUES (:userid, :group_chat_message, :groupid, :created_on)"
		;
